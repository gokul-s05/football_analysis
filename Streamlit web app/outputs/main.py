...
def main():
    ...
    demo_selected = st.sidebar.radio(label="Select Demo Video", options=["Demo 1", "Demo 2"], horizontal=True, index=0)
    ...
    # Ensure demo_selected has a valid value
    if demo_selected is None:
        demo_selected = "Demo 1"  # Default to Demo 1 if None
    ...
    model_keypoints = YOLO("fieldkeypoints.pt")
    ...
    with tab3:
        ...
        with bcol2:
            ...
            with bcol21t:
                show_k = st.checkbox(label="Show Keypoints Detections", value=False)

                show_p = st.checkbox(label="Show Players Detections", value=True)
            with bcol22t:
                show_pal = st.checkbox(label="Show Color Palettes", value=True)
                show_b = st.checkbox(label="Show Ball Tracks", value=True)
            plot_hyperparams = {
                0: show_k,
                1: show_pal,
                2: show_b,
    ...
    if start_detection and not stop_detection:
        st.info("Detection Started!")  # or st.success(), st.warning(), etc.
